...
*   **Güvenli Altyapı:** FastAPI backend ve PostgreSQL veritabanı entegrasyonu.
...
3.  `.env` dosyasını oluşturun ve gerekli yapılandırmaları yapın (örn. `.env.example` dosyasını referans alabilirsiniz).
    *Not: `DATABASE_URL` değerini kendi PostgreSQL sunucunuza göre ayarlayın.*
4.  Veritabanı tablolarını oluşturmak için migrasyonları çalıştırın:
    ```bash
