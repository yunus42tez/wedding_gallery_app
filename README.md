...
3.  `.env` dosyasını oluşturun ve gerekli yapılandırmaları yapın (örn. `.env.example` dosyasını referans alabilirsiniz).
    *Not: Varsayılan olarak SQLite kullanılmaktadır ve veriler `data/` klasöründe güvenle saklanmaktadır.*
4.  Backend sunucusunu başlatın (Veritabanı tabloları ilk çalıştırmada otomatik oluşturulacaktır):
    ```bash
